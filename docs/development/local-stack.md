...
in a browser on your machine, sign in, and try **Ask**.

You do **not** run a demo application on your laptop for that. `stack.sh` starts Brieve
only. Do not look for a second compose project or a command that starts `brieve-demo`
as a local app.

**Build is different.** Ask answers questions. Build’s finished result is a **GitHub
pull request** on some other repository — the application you want changed. That
repository must already exist on GitHub, and the **Brieve GitHub App** must be
installed on it. The usual subject is `brieve-demo` (a sample app repo). You present
its GitHub URL in the portal; you still do not start it as a program on your machine.
...
Optional later, not required to start the stack:

- An Anthropic API key, if you want Ask (and Build) to call a live model instead of
      refusing.
- A GitHub repository with the Brieve GitHub App installed, if you want to test Build
      through to a real pull request (often `brieve-demo`).
...
| Ask refuses before answering | Bring-up succeeded. Ask needs model settings in `.env` (next section). That is optional. |
...
| Build runs but ends without a pull request | The Brieve GitHub App is not installed on that repo, or the App key is not in Vault. The local demo app being stopped is not the cause. |
...
Restart is not always required for the next Ask if the API job already reads `.env`;
if a question still refuses, restart the API job (`nomad job status api`, then
`nomad alloc restart <allocation-id>`) or run `bash deploy/local/stack.sh up` again.

---

## Test Build all the way to a pull request

The portal can open without any of this. **Ask** can work with only the model settings
above. **Build** that produces a pull request cannot: the agent has to clone a real
Git repository and push a branch using the Brieve GitHub App. There is no substitute
subject on your laptop.
...
### 4. Put the App private key in Vault (once)
...
After the stack is up, from the `brieve` folder, with the `.pem` file your team gave
you (a path on your computer, not pasted into chat):
...
vault kv put -mount=harness-authority authoring/vcs-app \
  ...
  private_key=@/path/to/the-app.pem
```

`source .env` loads the App IDs and the Vault address the stack wrote. Replace
`/path/to/the-app.pem` with the real file path.
...
1. Set the model lines in `.env` as in the previous section, if you have not already.
   Build needs a model the same way Ask does.
2. Open **https://127.0.0.1:8082/**, sign in, stay on **Build**.
3. Describe the change **and include the repository URL**, for example:
   `https://github.com/mcteer/brieve-demo`
4. Submit. Wait for a pull request link. That link is the test passing.

If Build stops without a pull request, read the reason on the page. The usual causes
are: the URL is not on `PROPOSE_OWNED_REPOSITORIES`, the App is not installed on that
repo, the Vault key was never seeded, or no model is configured.
