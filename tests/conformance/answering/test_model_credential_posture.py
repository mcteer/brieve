...
def test_a_fixture_model_never_reaches_the_credential_store() -> None:
    ...
    source = inspect.getsource(entrypoint._chooser_from_model)  # noqa: SLF001 — the branch IS the claim
...
def test_the_run_path_fetches_after_the_cell_is_validated_and_before_anything_is_built() -> None:
    ...
    chooser_for = inspect.getsource(entrypoint._chooser_for)  # noqa: SLF001
    from_model = inspect.getsource(entrypoint._chooser_from_model)  # noqa: SLF001
    # 047 extracted credential fetch into `_chooser_from_model` so the analyzer and judge
    # share one builder. The sequence is still: validate the cell, then fetch, then build.
    assert chooser_for.index("resolve_bound_model") < chooser_for.index("_chooser_from_model")
    assert (
        from_model.index("FIXTURE_PROVIDER")
        < from_model.index("BrokeredModelCredential")
        < from_model.index("build_chooser(")
    )
