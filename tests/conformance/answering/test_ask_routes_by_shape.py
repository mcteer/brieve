...
def test_row_a_question_fitting_no_source_declines_naming_both() -> None:
    """The spec's *"decline, not a coin flip"* edge case, at the surface.

    And it reads no records on the way — a coerced route here would be the guidance→estate
    ...
    assert body["source"] == "neither"
    assert "corpus" in body["declined_reason"] and "records" in body["declined_reason"]
    assert _access_count(surface) == before
...
def test_row_the_route_is_recorded_on_every_ask() -> None:
    ...
    assert sources == ["guidance", "estate", "neither"]
