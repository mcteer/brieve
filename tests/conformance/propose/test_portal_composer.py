...
def test_p7_propose_template_has_no_agent_select() -> None:
    ...
    html = (root / "src/surfaces/portal/templates/_thread_composer.html").read_text(
        encoding="utf-8"
    )
...
def test_p7_run_page_shares_the_build_rail() -> None:
    ...
    assert 'include "_propose_run_main.html"' in html
...
def test_p8_ask_has_no_propose_control() -> None:
    """Ask cannot open a PR (047 P8) — default submit is Ask, not Build."""
    root = Path(__file__).resolve().parents[3]
    html = (root / "src/surfaces/portal/templates/_thread_composer.html").read_text(
        encoding="utf-8"
    )
