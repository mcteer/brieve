...
**This row was red, and what it found became 052.** Correcting the matcher (2026-08-27) made a
real finding visible: `scrub_authoring_requests` cleared `intents.arguments` at terminal state
and nothing cleared `checkpoints.payload`, which carried the same authored file bodies. 041's
FR-033 closed one copy of the content and left the other, for two features.

Closed by 052 (issue #219): the payload is scrubbed at terminal state, six pre-existing
checkpoints were backfilled, and this row is the acceptance signal that says so.
"""
