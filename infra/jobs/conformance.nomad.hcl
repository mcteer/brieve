  ...
  group "durability" {
    ...
    task "pytest" {
      ...
      config {
        ...
        # or it does not run.
        args = [
          "set -e; cd /repo; export PYTHONPYCACHEPREFIX=/tmp/pycache; pip install --quiet --disable-pip-version-check uv; uv run --extra adapters --extra surfaces pytest tests/conformance/durability tests/conformance/api tests/conformance/identity -m \"not host_enclave\" -q"
        ]
      }
