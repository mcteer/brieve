...
- [ ] T001 Write Accepted ADR-0071 from research R5 in
...
- [ ] T003 Add frozen `AgentPin(phase, path, version, digest)` and
      `PackManifest.agents: tuple[AgentPin, ...]` in `src/core/packs/manifest.py` (parallel
      to `SkillPin`; export in `__all__`)
- [ ] T004 Parse `[[agents]]` in `src/core/packs/loader.py` `parse_manifest` /
...
- [ ] T005 Extend `FilesystemPackLoader._verify_digests` in `src/core/packs/loader.py` to
...
- [ ] T006 Implement `load_phase_agents(pack_name, phase, *, loader, packs_root) -> PhaseAgents`
...
- [ ] T007 Add `instruction: str = ""` to `ChoiceRequest` in `src/core/choice/chooser.py`;
      default empty for Ask
- [ ] T008 Pass `instruction` into `ChoiceRequest` from `resolve_step_tool` in
      `src/core/choice/bounded.py` (new parameter default `""` so existing callers stay valid)
- [ ] T009 Implement `bind_phase_agents(run, phase) -> PhaseAgents` in
...
- [ ] T012 Wire `bind_phase_agents` at each 047 phase start in
...
- [ ] T013 [P] Component tests for `AgentPin` parse/load in
...
- [ ] T019 [P] [GATE:conformance] Ask never sets `ChoiceRequest.instruction` from
      `packs/*/agents/` in `tests/conformance/phase_agents/test_ask_does_not_use_build_agents.py`
      (new) (A8, FR-014)
- [ ] T020 [P] Unit gate: `src/core`, served `src/adapters`, and `src/surfaces` do not
...
- [ ] T023 [P] [US1] Author five distinct Terraform phase files
...
- [ ] T026 [P] [US2] [GATE:fail-closed] Vault omit-Write fails closed in
      `tests/conformance/phase_agents/test_vault_missing_write_fail_closed.py` (new)
- [ ] T027 [P] [US2] [GATE:conformance] Isolation A2/A3 in
...
- [ ] T028 [P] [US2] Author five distinct Vault phase files
...
- [ ] T031 [P] [US3] [GATE:conformance] SC-001 body-distinctness (not path-only) in
      `tests/conformance/phase_agents/test_agents_bodies_differ_by_phase.py` (new)
- [ ] T032 [P] [US3] Assert authoring-tier / bind path never calls out to the public web
...
- [ ] T033 [P] [US3] Rewrite Terraform `packs/terraform/agents/*/AGENTS.md` from current
      ...
      (FR-006, FR-007)
- [ ] T034 [P] [US3] Rewrite Vault `packs/vault/agents/*/AGENTS.md` from current public
...
- [ ] T035 [US3] [GATE:conformance] Mechanical checks that Terraform Write mentions
...
- [ ] T042 [US4] Add optional extra `prompt-tune` in `pyproject.toml`:
...
- [ ] T047 [P] [GATE:conformance] A13: portal templates/JS do not contain phase instruction
...
- [ ] T048 [GATE:conformance] Named-runner checklist for E1–E3 in
      `specs/049-phase-product-prompts/contracts/conformance-phase-product-prompts.md`
      (record who ran it on the implementation PR; **named runner: Dan McTeer**)
- [ ] T049 [P] Update pack layout in `packs/README.md` to include `agents/<phase>/AGENTS.md`
      + `PROVENANCE.md`
- [ ] T050 [P] Add glossary terms for pack phase `AGENTS.md` vs repository-root contributor
      `AGENTS.md` in `docs/glossary.md`
- [ ] T051 [P] Changelog entry for user-visible Build steering in `CHANGELOG.md`
- [ ] T052 [P] ROADMAP row/note for 049 in `ROADMAP.md` if that table is the feature index
- [ ] T053 Re-run `tests/conformance/packs/test_core_is_product_blind.py` and T020; fix any
      product identifier or `dspy` leak in `src/core`
- [ ] T054 Security-maintainer review request for `PackManifest` / `ChoiceRequest` /
