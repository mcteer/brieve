...
| A3 | A terraform-bound run's `content_pins` contain `terraform/agents/<phase>` and no `vault/agents/*` | Vault keys present or terraform keys absent after Research |
...
| A11 | `phase_agents` and `build_agents` each include at least one fixture that scores fail | Suites with only passing cases |
| A12 | `promote_phase_agents` refuses when either qualification is missing | Promote returns evidence without both suites |
...
| E3 | SC-006: promoted Terraform and Vault full Builds beat generic pre-feature steer on the authoring corpus (eval, not pytest wording) | Dan — eval lane |
