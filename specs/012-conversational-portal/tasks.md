...
- [ ] T005 Add `TURN_RECORDED` and `THREAD_DELETED` to `AuditEventType` in `src/core/audit/schema.py` with docstrings carrying the D4 reasoning (a declined ask is an ask; the trail is the record). This is the feature's one sealed-core change — note the spec approval + reviewer in the commit
...
- [ ] T008 [P] Create `src/core/threads/schema.sql`: `threads`, `thread_turns` (with the `(tenant_id, subject_user_id, created_at)` index for the rate window and keyset listing), `run_inputs` — idempotent, every statement `IF NOT EXISTS`
...
- [ ] T012 Create `src/core/threads/turns.py`: `accept_turn()` implementing the contract's ordered semantics — resolve thread → rate window (30/5min from the store) → compute context (5 most recent dispatched-with-results, record drops) → **write `TURN_RECORDED` first** → decide (decline `nothing_to_dispatch` / refuse `not_permitted` via 011's `may_start` intersection / write `run_inputs` and dispatch through the same core path `start_run` uses) → append turn row. Reasons extend 011's frozen `OPERATION_REASONS` mapping with `rate_limited`
- [ ] T013 [GATE:correlation] Component rows in `tests/component/test_turn_evidence_first.py`: every disposition — dispatched, declined, refused, rate-limited — has a `TURN_RECORDED` event written **before** any dispatch or row append; the event carries message verbatim, disposition, reason, context fields. Break the decline branch's event write and watch the row fail, then restore
- [ ] T014 [GATE:fail-closed] Component rows in `tests/component/test_turn_decision.py`: no agent selected → declined, no run; not-startable agent → refused `not_permitted`, no run; over rate window → refused `rate_limited`, no run; a store error during acceptance refuses rather than dispatching (FR-017a — the fail-closed direction is "no run", never "run anyway")

### Run input reaches the run

- [ ] T015 Extend `src/surfaces/dispatch/entrypoint.py`: read `run_inputs[run_id]` from Postgres under workload credentials; resolve `context_run_ids` to recorded results **verbatim** from the durability store at run start; include message + received context in what the scripted run records (its result carries `received_context` so rows can byte-compare). Absent input row = a run started outside a thread — proceed exactly as today
- [ ] T016 [P] Component row in `tests/component/test_run_receives_input.py` (in-process dispatcher + in-memory store): a dispatched turn's run receives the message and each context result byte-identical to the recorded original (SC-002's word is "byte-identical" — compare bytes, not lengths)

### Five operations, both transports, snapshot-first

- [ ] T017 [GATE:conformance] Grow `specs/008-northbound-api/contracts/operations.snapshot.json` with `POST /threads` (`create_thread`); watch parity fail; implement `src/surfaces/api/threads.py::create_thread` + MCP tool in `src/surfaces/mcp/operations.py`; watch parity pass. Repeat this loop — one operation per commit — for `send_turn` (T018), `list_threads` (T019), `get_thread` (T020), `delete_thread` (T021)
- [ ] T018 [GATE:conformance] `POST /threads/{thread_id}/turns` / `send_turn` in `src/surfaces/api/threads.py` + `src/surfaces/mcp/operations.py` (snapshot grown first), delegating to `accept_turn()` — the surface owns transport shape only; the decision lives in core, which is what makes verdict parity structural
- [ ] T019 [GATE:conformance] `GET /threads` / `list_threads` in `src/surfaces/api/threads.py` + `src/surfaces/mcp/operations.py` (snapshot grown first) — keyset cursor, no totals, 011's listing discipline verbatim
- [ ] T020 [GATE:conformance] `GET /threads/{thread_id}` / `get_thread` in `src/surfaces/api/threads.py` + `src/surfaces/mcp/operations.py` (snapshot grown first) — turns in seq order, run state joined from the durable record at read time, never stored on the turn
- [ ] T021 [GATE:conformance] `DELETE /threads/{thread_id}` / `delete_thread` in `src/surfaces/api/threads.py` + `src/surfaces/mcp/operations.py` (snapshot grown first) — `THREAD_DELETED` event first, then hard delete; runs and `run_inputs` untouched
- [ ] T022 Wire the thread store into both assemblies: `src/surfaces/api/app.py` `create_app(..., thread_store=...)` with the in-memory default (same rule as 011's collaborators), and `src/surfaces/mcp/transport.py` mirroring it
- [ ] T023 [P] [GATE:conformance] Verdict parity rows in `tests/conformance/mcp/test_surface_parity.py`: same inputs → same disposition on both transports for dispatch, decline, refusal, and rate limit — a surface that declines on one transport and dispatches on the other is two authorization paths wearing one name
...
- [ ] T025 Create `src/surfaces/portal/session.py`: in-memory store, opaque id → (subject, token, expiry); cookie `HttpOnly; Secure; SameSite=Lax`; sessions die with process or token expiry. **Deliberately not Postgres** — FR-020b's reasoning in the docstring
...
- [ ] T032 [P] Draft `docs/adr/0051-a-turn-is-evidence-a-thread-is-a-view.md`: every submitted message is trail evidence before anything else happens, including declines; threads are hard-deletable views; deletion is an event; the cost (permanent evidence of every message) stated and accepted; Status: Accepted (decided by this feature's spec + clarification)
