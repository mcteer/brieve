...
- [X] T001 Cherry-pick `fix/evidence-read-returns-the-newest-window` (commit `21e71a3`) onto the
...
- [X] T002 `EvidenceQueryRequest` in `src/core/audit/query.py` gains
      ...
      today's read.
- [X] T003 The in-memory twin in `tests/harness/memory_evidence.py`: when `limit_per_type` is set
...
- [X] T004 The Postgres implementation in `src/core/audit/postgres_query.py`: one windowed query —
...
- [X] T005 [GATE:fail-closed] Property rows in `tests/component/test_evidence_read_window.py`,
      **written once and parametrized over implementations** (the in-memory one runs hermetically;
      ...
      bound intact.
- [X] T006 [P] [GATE:conformance] The SQL-shape row beside the Postgres implementation: the
...
- [X] T007 [GATE:conformance] The mutation check, performed and **recorded in
...
- [X] T008 [US1] Grow `ESTATE_TERMS` in `src/core/answering/routing.py` with **plural-only
      ...
      **The first draft of this task was internally unsatisfiable, and the analysis pass measured
      ...
      task was written.
- [X] T009 [US1] [GATE:conformance] The routing rows in `tests/component/test_ask_routing.py`:
...
- [X] T010 [US2] Create `src/core/answering/focus.py`: `focus_types(question) ->
...
- [X] T011 [P] [US2] [GATE:fail-closed] The focus rows in `tests/component/test_estate_focus.py`:
...
- [X] T012 [US2] Wire the ask path in `src/surfaces/api/ask.py`: `estate_answer_for` computes
...
- [X] T013 [US2] The answer's `window_note` in `src/core/answering/estate.py`: present exactly
...
- [X] T014 [US2] [GATE:conformance] **The volume row** in
...
- [X] T015 [US2] [GATE:conformance] The window note on both surfaces: the API response and the MCP
...
- [X] T016 [US2] The portal rendering in `src/surfaces/portal/templates/ask.html`: the window note
