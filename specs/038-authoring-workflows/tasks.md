...
- [ ] T009e Make the handoff a **resume**: the `analyzer` calls `checkpoint_run` before it exits and the `proposer` calls `resume_run` (`src/core/durability/{checkpoint,resume}.py`), recording `RUN_RESUMED`. Row **T7** in `tests/conformance/authoring/test_tier.py` asserts step accounting, bounds and the lease re-acquisition all come from that path. **Not a bespoke carrier in `workspace.py`**, which the earlier draft specified: that would duplicate the durability seam (Principle VII) and route run state around it (Principle V seals durability), and would have had to reinvent re-observation next. The second process continues the first's run — which is what `resume` is for (research R32).
