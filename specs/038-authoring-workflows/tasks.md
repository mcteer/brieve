...
- [ ] T009e Make the handoff a **checkpoint and a continuation, NOT a resume**: the `analyzer` calls `checkpoint_run` before exiting; the `proposer` calls `provider.load(blob_id)` and continues under a freshly acquired lease. Row **T7** in `tests/conformance/authoring/test_tier.py` asserts step accounting and bounds cross, **`resume_count == 0` after a clean handoff**, and that the analyzer leaves the run **non-terminal**. **`resume_run` is deliberately not called** — measured, it counts `attempt = resume_count + 1` against `RESUME_ATTEMPT_CAP = 5` and stops the run terminally past it, so a designed-in handoff would spend attempt 1 of 5 on every healthy run and leave a genuinely interrupted one with four revivals where every other run type has five (research R35). Revival counting lives in `resume_run`, not `checkpoint_run`, so state transfers without touching a budget that exists to stop flapping runs.
