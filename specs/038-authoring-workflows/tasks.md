...
**Pass two found a different shape and it is worth stating at the top**: not things nothing
built, but **things built against the wrong subject**. The observer could never have found the
proposal (T044a); the `write` cell could never have been promoted (T061a); and two mechanisms
these tasks cited — `run_program`'s registration, `reachable_tools` — are not on the path they
claimed (T014, T039). All four arrived by **citing a real module** rather than by measuring
...
- [ ] T012aa Register `author_file` in `src/surfaces/toolset.py` **in this phase, before any ceiling names it**. `parse_ceiling_record` refuses a ceiling naming a tool outside the registered set, and `toolset.py` records that the vocabulary is *derived from what actually registered* — so a ceiling authored in T012c before the tool exists refuses `unknown_ceiling_entry`, and the error names the ceiling rather than the missing registration. (T014 then implements the handler this registers.)
