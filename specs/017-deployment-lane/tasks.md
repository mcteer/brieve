...
- [ ] T015 Harden `infra/bin/deployment-conformance` (created in T002) — **record which surfaces it started and stop exactly those when it finishes, on success or failure**, leaving any a developer had already brought up alone. Without this the surfaces persist and hold reservations on the NEXT invocation, competing with the conformance batch job at line 2 — which is verbatim the failure `infra/bin/portal-up`'s header records, and which FR-007 forbids. It is invisible in CI, where every run is a fresh runner and a single invocation, and it fails on a developer's second local run. Also force a **new allocation** rather than trusting `nomad job run` against an unchanged jobspec (research R7, observed 2026-07-31), capture and print the scheduler's placement output on failure, then invoke the rows
- [ ] T015a [GATE:fail-closed] Add `test_the_runner_leaves_no_footprint` to `tests/conformance/deployment/test_every_declared_process_is_asserted.py` — assert the runner stops every surface it started, so a second consecutive `make conformance` finds the same free capacity as the first. Asserted rather than trusted, because the symptom is a placement failure two runs in that reads as an infrastructure problem rather than as a gate defect
