...
- [ ] T015 Harden `infra/bin/deployment-conformance` (created in T002) — force a **new allocation** rather than trusting `nomad job run` against an unchanged jobspec (research R7, observed 2026-07-31), capture and print the scheduler's placement output on failure, then invoke the rows
...
- [ ] T036 [GATE:conformance] Record the collected-row count on `main` **before** merging, then run `make conformance` on this branch and confirm every one of those pre-existing rows still runs. Comparing totals proves nothing — this feature adds rows, so the total rises whatever happens to the old ones. A lane that silently stopped collecting a directory looks exactly like a pass (FR-007, SC-005)
