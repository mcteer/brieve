...
- [ ] T015 Harden `infra/bin/deployment-conformance` (created in T002) to implement FR-007a's lifecycle in full — reuse what is already running and record only what it started; **stop exactly those on a passing run**; **on failure leave them standing and say so** ("left `api` running for inspection; run `make deployment-down` to clear it"), because the allocation is what someone needs to diagnose and a captured log tail is often not enough; and **fail the gate if a stop fails** rather than swallowing it. Also force a **new allocation** rather than trusting `nomad job run` against an unchanged jobspec (research R7, observed 2026-07-31), and capture and print the scheduler's placement output on failure
- [ ] T015b [GATE:fail-closed] Reclaim leftovers **early**: add a step at the start of the `conformance` target in `Makefile` — or in `infra/bin/enclave-conformance`'s existing purge phase — that stops any deployment surfaces a previous failed run left standing, and add `make deployment-down` as the command the failure message names. FR-007a clause 4: the gate runs **last**, so it cannot reclaim capacity before the conformance batch job needs it at line 2. Locally a prior failure is the normal case — the gate is re-run precisely because it failed — so without this, one red run starves the merge-blocking checks on every subsequent one
- [ ] T015a [GATE:fail-closed] Add `test_the_runner_leaves_no_footprint` to `tests/conformance/deployment/test_every_declared_process_is_asserted.py` — assert FR-007a's lifecycle in all three directions — on a passing run every started surface is stopped, so a second consecutive `make conformance` finds the same free capacity as the first; **a stop that fails, fails the gate** rather than being swallowed; and **leftovers from a failed run are reclaimed before the conformance batch job is submitted**, not after. The happy path alone would leave the requirement unenforced in exactly the two cases that reintroduce the defect. Asserted rather than trusted: the symptom is a placement failure two runs in that reads as an infrastructure problem rather than as a gate defect
...
Phase 2 (Foundational)   T004–T007 [P] → T007a → T008 → T008a
                         T009 → T010 → T011 → T012, T013, T014 → T015 → T015b → T015a → T016
                             ↓
        ┌────────────────────┼────────────────────┐
        ↓                    ↓                    ↓
