...
**Tests**: Included and mandatory — spec Independent Tests, contract rows A1–A18 / E1–E3, and
...
| Gate type | Where |
| --- | --- |
| **Fail-closed** | T015, T017 — a save failure stops the run with the reason recorded, asserted **against the stored row** rather than the return value, because a scrub that reported a count while leaving the row intact is the failure nothing can detect afterwards |
| **Conformance** | T008–T013 (A1–A3, A6–A8), T016–T020 (A9, A17, A18, A11), T021–T024 (A4, A5, A10), T026–T029 (A12–A14), T032–T034 (A15, A16); T036 named-runner E1–E3 |
| **Correlation / evidence** | **T018 — every column survives the re-save, including the correlation ID**; T026–T029 — a scrubbed run still compiles a RunReport that names every authored path and keeps its pull request identifiable |
| **No-secret-leak** | The whole feature. T035 is the acceptance sweep: the live store holds no authored body after a completed run |
...
- [ ] T014 [US1] Call `scrub_proposal_payload` in `src/surfaces/dispatch/entrypoint.py` at the
      ...
      **Construct the blob from the one being rewritten, replacing `payload` and nothing else.**
...
- [ ] T018 [US1] [GATE:correlation] **Assert the re-save carries every column** (row A17):
      ...
      correlation ID
- [ ] T019 [P] [US1] [GATE:conformance] Assert a **non-authoring** run's payload is untouched
...
- [ ] T020 [P] [US1] [GATE:conformance] **Assert a refused run's payload contains no authored
...
- [ ] T021 [US3] [GATE:conformance] **Assert the analyzer branch does not scrub the payload**
...
- [ ] T022 [US3] [GATE:conformance] Assert a **failed** publish leaves the payload intact, so
      the resumption has what it needs (row A5, FR-002)
- [ ] T023 [P] [US3] [GATE:conformance] Assert `proposal_from_payload` **raises** on a scrubbed
...
- [ ] T024 [US3] Assert a run killed between Judge and publish resumes and opens a pull request
...
- [ ] T025 [US2] Verify no consumer breaks: `PROPOSAL_PAYLOAD_KEY` has exactly two readers —
...
- [ ] T026 [P] [US2] [GATE:correlation] Assert a RunReport compiled from a scrubbed run
      validates and names every authored path (row A12, FR-003, SC-002)
- [ ] T027 [P] [US2] [GATE:correlation] Assert the pull request stays identifiable from a
      scrubbed run's record (row A13, FR-004)
- [ ] T028 [P] [US2] [GATE:evidence] Assert the compiled report does not claim to carry content
      the run no longer holds (row A14)
- [ ] T029 [US2] Assert a report compiled before and after the scrub agrees on paths and
...
- [ ] T030 Write `scripts/backfill-proposal-payloads.py`: one-time, idempotent, operator-invoked.
...
- [ ] T031 Scope the backfill to **terminal checkpoints only**. A non-terminal one may still be
      resumed, and scrubbing it is the same defect T021 guards against, arriving by a different
      route
- [ ] T032 [P] [GATE:conformance] Assert the backfill leaves non-terminal checkpoints intact
      (row A15)
- [ ] T033 [P] [GATE:conformance] Assert the backfill is idempotent and reports what it changed
      (row A16)
- [ ] T034 Run the backfill against the live store and record the before/after count. **Six
...
- [ ] T035 [GATE:no-secret-leak] Confirm
      `tests/conformance/durability/test_dispatched_no_secret_sweep.py::test_row_checkpoints_still_hold_no_credential_material`
      **passes** (SC-006, FR-014 — the row must still assert credential *shapes* rather than
      words), and remove the KNOWN RED note its docstring carries. That row is this
      feature's acceptance signal and closes issue #219
- [ ] T036 [GATE:conformance] Named-runner rows on the implementation PR — **Dan McTeer**:
      ...
      **E2** the acceptance sweep over the live store after the backfill, recording the
      ...
      same files
- [ ] T037 [P] Correct `proposal_payload`'s docstring in `src/surfaces/dispatch/authoring.py`.
...
- [ ] T038 Record the never-terminal gap where a reader of the platform will find it (FR-011):
...
- [ ] T039 [P] Add a `CHANGELOG.md` entry: a finished authoring run no longer leaves its
...
- [ ] T040 [P] Add `proposal payload scrub` to `docs/glossary.md`, stating what is cleared, what
...
- [ ] T041 Add the ROADMAP Shipped row for 052 **on the day it merges**. The file has been three
      features behind four times, warns about exactly that in its own text, and is what a
      planner reads first
- [ ] T042 Run every scenario in [quickstart.md](quickstart.md)
- [ ] T043 Run `make check`, then `make conformance`, then `make test-full`
- [ ] T044 Request **security-maintainer review** on the implementation PR.
...
- **Phase 6 (Polish)** — T030–T035 depend on Phase 2 (the function) and Phase 3 (the call site);
  T034 must precede T035, because the row cannot pass while the six rows remain

### Within phases

T003 → T004 → T005 → T006 → T007 (the function, then its properties, then its record).
T014 → T015 → T016. T030 → T031 → T034 → T035.
...
- T017 ‖ T020 (US1 tests; T016 follows T014)
- T023 ‖ (T021, T022, T024 touch the same call-site behaviour)
- T026 ‖ T027 ‖ T028 (US2 rows)
- T032 ‖ T033; T037 ‖ T039 ‖ T040 (Polish)
...
One CRITICAL and four others, all applied. Recorded because a regeneration from an older spec
would reintroduce every one.

**CRITICAL — the re-save would have blanked the correlation ID.** `save()` overwrites the whole
row. `run_state`, `stop_reason` and `resume_count` carry guards, each added after somebody lost
that column; `correlation_id` does not. The obvious implementation of "persist the scrubbed
payload" destroys the join attestation is walked along — in the feature whose US2 exists to keep
runs attestable, and no other row would notice, because every other row reads the payload.
T014 states the rule; **T018 reads the columns**.

**Resolved in `spec.md`:**

- **FR-007** asked for something unobservable. A refused run never composes a proposal, so a row
  asserting the scrub cleared it would pass without exercising anything. Restated as the
  property that can fail; T020 ships it.
- **FR-014** described work that had already merged in #220. Restated as the non-regression
  obligation that remains; T035 discharges it.
- **FR-015** added for the backfill, which the plan designed and no requirement asked for.
  T032–T034 build it.
- **FR-011** sharpened: its obligation was dischargeable only by this document, which nobody
  reads while reading the code. T038 records the gap beside the scrub and in the ROADMAP.

**FR-012 gained a row.** The scoping is structural — the call sits inside the `PROPOSER`
branch — and T019 asserts it anyway, because a structural property is what somebody undoes by
hoisting a call one line out of a branch with every other row still green.
