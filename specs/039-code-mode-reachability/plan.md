...
| **Durability gains a column** — `intents` carries the arguments an invoke was about to run with (FR-015) | Resume **re-invokes** a pending step, and once the arguments are the model's rather than a platform constant there is nothing to re-invoke with. Every model-driven run would resume with an empty map, having correctly skipped the provider call | **A second store** is what the entrypoint already argues against for the tool name — *"a second store holding the same fact would eventually disagree with it."* **Re-asking the model** is what `already_chosen` exists to prevent. **Storing a hash** leaves nothing to re-derive from |
| **Raw model-supplied argument values rest durably**, for the first time anywhere (FR-016) | FR-015 requires re-invoking with the actual arguments, and `redact_arguments` returns *"argument keys and content hashes — never raw values"*, applied at `engine.py:101` to every invoke. A hash cannot be re-invoked with, so the rule has to break somewhere | **Nowhere** means no resume for model-driven runs. **The trail** is the one place it must never be: `record_choice` argues that rule for itself, and an append-only store is where a leaked secret cannot be taken back. Bounded instead — control plane, one open bracket, removable with the run, and a row asserts nowhere else |
| **The recording format grows a second grammar** (FR-017) | A recording must be able to carry arguments, and `parse_recording` is `raw.split(",")` on an environment variable while JSON contains commas — so the obvious encoding breaks the separator four existing suites depend on | **One widened grammar** makes every existing caller move, which is the blast radius the byte-identical discipline exists to refuse. **Encoding arguments so no comma appears** preserves one grammar and produces a fixture nobody can read — and a recording is written by hand |
| **A capability is refused rather than bounded** — a program may not submit a program (FR-018) | The seam has no special case, so a definition whose ceiling names the program tool can recurse, and the demonstration definition does. Absent from 036's Deferred list, it is permitted by **omission**; shipping reachability makes that live, bounded only by the step budget | **A depth limit** is a decision about how much nesting is useful and there is no evidence about that, so a depth of 1 pretends to an answer. **Leaving it** ships a capability nobody argued for. A refusal is reversible by a later record; an unbounded recursion that shipped is not |
...
third is a narrowing that leaves every existing key byte-identical. **The last four are the second's
blast radius**, traced one hop at a time across three analyze passes and recorded here together: widening what
a model may answer reaches resume, redaction, the recording format, and — through the ceiling — nesting.
Only the fifth is a security posture change, and it is the one to read.
