...
row — twenty-one of them, K1 through K15 — has a task, and **every task that asserts has a task that builds**. That pairing
...
| Gate type | Where |
| --- | --- |
| **Fail-closed** | T012 (an absent runtime refuses with a stated reason), T020b (a looping program writes one intent per effect), T020b1 (a program cannot submit a program), T023 (an exhausted bound ends the RUN) |
