...
- [ ] T028 [US4] [GATE:fail-closed] Implement the stop operation in `src/surfaces/api/runs.py` + MCP: verify the caller **started** this run via the run index (FR-010 — its only role here; the index is insert-only per its own contract and the stop does not touch it), then write `run_state = STOPPED`, `stop_reason = stopped_by:<subject>` to the **checkpoint** through `PostgresDurabilityProvider.save` — state lives on the checkpoint, which is the whole reason the index could stay insert-only. Stopping a terminal run reports the existing state (FR-011) — asking twice is not an error. The caller returns immediately; nothing waits
...
**42 tasks** — 2 setup, 7 foundational, 6 US1, 6 US2, 5 US3, 6 US4, 5 US5, 5 polish.

One was added by analyze pass 1 with a finding behind it: T029a, because the pass read
`save()`'s SQL and found the research asserting the opposite — the shipped upsert is
last-write-wins, so a routine checkpoint would have erased a stop and resurrected the run.
