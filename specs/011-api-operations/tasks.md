...
- [ ] T012 [US1] [GATE:fail-closed] Implement collect in `src/surfaces/api/mappings.py`: resolve the record by accessor **through the T009a change-request store**, refuse unless the caller is its requester in its tenant (caller sees not-found, trail records `outside_tenant`), then poll Vault. **Pending is an answer, indefinitely** — never presented as failure
...
- [ ] T024 [US3] [GATE:fail-closed] Implement retrieval in `src/surfaces/api/runs.py` + MCP: resolve through the **T009a index reader** (the tenant check), read the terminal checkpoint **via the T009a provider**, compute the **three-way disposition** (FR-007) — no terminal state → not finished; result key → the result; terminal without it → ended without one, with `stop_reason` as the why. Never the raw payload
