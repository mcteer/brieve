...
| V — Sealed Core, Versioned Seams | **Pass, three named touches** | (1) **Durability**: `intents` gains a nullable `arguments` column, additive, NULL meaning pre-feature (R4); threaded through the one `IntentRecord` constructor and all three Postgres column lists. (2) **Registries**: `register()` gains `max_request_bytes` with a platform default — additive keyword, existing registrations unchanged. (3) **Adapters**: `ModelChooser`'s answer widens; all four `Chooser` implementations are in-repo (R9). **The idempotency key is untouched** — 040 has no programs, so keys are already distinct (R2). The engine is edited at exactly one line: `bracket_call`'s call site passes through the arguments the engine already holds — no derivation, no decision, no new state. All three carry security-maintainer review. |
