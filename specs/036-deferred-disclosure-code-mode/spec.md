...
- **A program that fails partway.** Calls already made had real effects. The run must be resolvable by observation rather than by guessing, exactly as an interrupted structured run is.
...
- **FR-010**: Execution bounds that apply to a run MUST apply to a program's calls at the same grain they apply to structured calls, and MUST be set by the platform rather than by the program.
