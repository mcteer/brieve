...
- [x] T001 Vendor the official HashiCorp logomark into
...
- [x] T002 Implement `history_items` in `src/surfaces/portal/app.py` per
...
- [x] T006 [US1] In `tests/component/test_portal_shell.py`: signed-in `GET /`
...
- [x] T010 [US1] Rounded composer + Ask/Build slider in
...
- [x] T011 [US1] In `src/surfaces/portal/static/portal.css` paint column,
...
- [x] T012 [US3] In `tests/component/test_portal_shell.py`: a fixture with one
...
- [x] T013 [GATE:fail-closed] [US3] In `tests/component/test_portal_shell.py`:
      unreachable conversations omit Ask rows and show a notice (not “no
      conversations”). Unreachable builds do the same for Builds.

- [x] T033 [P] [US3] In `tests/component/test_portal_shell.py`: + is named,
...
- [x] T014 [US3] Add `src/surfaces/portal/templates/_history.html`: search field
...
- [x] T015 [US3] Add `src/surfaces/portal/static/portal-history.js`: filter
      `[data-history-row]` by visible text; no-matches copy; no API call. Absent
      script leaves the full list.

- [x] T016 [US3] In `src/surfaces/portal/templates/base.html`: **+ New** is
...
- [x] T017 [US2] In `tests/component/test_portal_asks.py`: Ask Stop / abort copy
      does not say the answer ended (research F5).

- [x] T018 [US2] In `tests/component/test_portal_shell.py`:
...
- [x] T019 [GATE:fail-closed] [US2] In `tests/component/test_portal_shell.py` /
...
- [x] T020 [US2] Keep Ask abort in `src/surfaces/portal/static/portal-ask.js`;
      after a first land, `history.replaceState` still goes to `/ask/{id}`. Do
      not add an ask-cancel API.

- [x] T021 [US2] Move the existing stop form into the bubble on
...
- [x] T022 [US2] On an open Ask in `src/surfaces/portal/templates/ask.html`,
...
- [x] T023 [US4] In `tests/component/test_portal_identity.py`: bottom column
      exposes accessible names Settings and Sign out (or logout) and
      `href="/settings"`; profile shows `subject_user_id`.

- [x] T024 [US4] In `src/surfaces/portal/templates/base.html`: profile
...
- [x] T025 [US5] Extend `tests/component/test_portal_identity.py`: every colour
...
- [x] T026 [US5] Update a11y fixtures/selectors in `tests/a11y` for the new
      column (New, Projects, search, slider, Stop, +, profile, Settings,
      logout). Lane stays `color_scheme="dark"` only.

- [x] T027 [US5] Confirm 24×24, 320px reflow, visible focus, and skip-to-content
...
- [x] T028 Remove the icon rail from `src/surfaces/portal/templates/base.html`
...
- [x] T029 Rewrite F9 decision-comments in `src/surfaces/portal/templates/base.html`
...
- [x] T030 [P] Changelog entry for the create-home shell in `CHANGELOG.md`. Do
      not add a glossary product name from the reference screenshot.

- [x] T031 [GATE:conformance] `make check` and
