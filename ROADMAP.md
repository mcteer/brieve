...
| author the changes | 038's `author_file` / `read_subject`, containment, provenance | **still registered nowhere** — `src/surfaces/handlers.py` holds four Vault and Terraform tools and none of the trio. **The successor feature**, and 040's capability ledger now names all three in `DELIBERATELY_UNREACHABLE` against this feature as their record, so the gap is held by a gate rather than by memory |
