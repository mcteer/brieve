...
| 042 | Vault policy authoring, end to end | **ADR-0068 (new)** — impact is measured by the product, in a namespace reserved for measurement. **ADR-0038/0064/0066 consumed unchanged** (041’s tier is product-blind and stayed that way — one generic `evidence` field on `Proposal` is the only edit inside it, asserted by a diff row), ADR-0025 (registry isolation, now in policy space), ADR-0047 (the reason Vault is first and Terraform is not), ADR-0004 (the pinned Vault operating guides as grounding), Principle IV (a run gains its first WRITE capability against the trust fabric, bounded by namespace rather than by withholding the verb) | **The change-proposal workflow’s first product, and the first thing in this estate ever permitted to write to Vault through the workload identity** — `vault_write` was a stub returning `written: True` without touching the product, and `agent_pack_secrets` deliberately carried no write grant. The measurement is one tool call because splitting it would make “always destroyed” depend on a model choosing to call the last one: a model can request a *measurement*, not a *policy*. **The central refusal is three independent layers** — request validation, a GOVERNANCE hook, and Vault’s own ACL — and V3 deletes the middle one to prove the safety case can lose, while V16 exercises the product layer with the platform’s hook disabled, because a back-stop only ever tested behind a working front-stop has never been tested. **Findings the tooling produced rather than review**: `terraform validate` refused to plan a protected set referencing `agent_ceiling.name` — it is `for_each` over the agent definitions, so the set was silently missing *every* ceiling, the most important names in it; the pack manifest refused the whole vault pack when a tool was declared one commit before its handler existed, catching an ordering error in this feature’s own task list; and the analyze pass’s U1 was real — `harness_authority_read` enumerates its subpaths, so the new record got no grant and the 403-not-404 trap already recorded twice in that file would have reported it as an unreachable fabric. **The live probe earned its keep on the first run**: Vault answers `["deny"]` for an unreachable path, and the raw arithmetic reported `granted: ["list"], revoked: ["deny"]` — the absence of capabilities spelled as a fact, making a reviewer count one grant twice. No hermetic row caught it because the scripted Vault never returned the real marker. **Owed**: requester-scoped reads (reads run on the allocation’s platform identity, so evidence may describe more of the estate than the requester is entitled to know — ADR-0044 territory, unbuilt); and **the impact is measured against OUR Vault, not the target’s** — found 2026-08-07 when PL2’s real proposal against `mcteer/vault-validation` turned out to be unmergeable. It proposed KV **v2** paths for a mount that repository enables with `-version=1`, and scoped to a prefix the benchmark does not write to. The measurement was accurate about what the policy would permit in the dev enclave and blind to the estate the change was for, so ADR-0068’s *“measured by the product”* holds only where the measuring product IS the target. A proposal against somebody else’s Vault needs either their mount’s shape as an input or a disclosure that the measurement did not cover it — the second is cheap and honest, and neither exists |
| 045 | Customer-supplied context — endorsed, pinned, citable | **ADR-0070 (new)** — endorsed-content sync is an enumerated egress class, and customer content is *consulted* material handled by the *pinned* mechanism (Principle II requires an ADR to add a class; ADR-0030's tension settled out loud rather than left describing a mechanism the tree does not use). **ADR-0069 consumed unchanged** (the fourth console record rides the same request-and-decide path — no second write mechanism), ADR-0004 (why endorsement is needed at all: the supply chain cannot vouch for a customer's own documents), ADR-0021 (the date-stamped-snapshot shape), ADR-0046 (the tenant key exists from day one; the wall is still its feature), ADR-0066, ADR-0047 | **The design decision was the feature.** A `tenant_id` threaded through `load_corpus` would have made one reader serve two trust models, and every check on the pinned side would have grown a branch for the endorsed side. Instead: a second corpus behind the same contract, and `corpus.py` untouched — asserted as a **diff row that lands first**, so it speaks about every later commit rather than none. **The version identity is over content and NOT over the upstream tip**, because a force-push would otherwise silently change what a pinned version means. **Four findings the tooling and the live legs produced rather than review**: `test_the_cli_is_withdrawn` refused the planned `src/surfaces/sync/` on the first run — `surfaces/` holds the three transports plus the dispatch seam, and this is the one place the platform reaches *out*; the anchor slug had to match GitHub's actual rule (runs left by removed punctuation are **not** collapsed, underscores kept), because a cleaner anchor resolves here and 404s in the customer's own browser; running EL1 against a real repository found the citation URL was `location + "/" + relative`, giving `…/standards.git/logging.md` — **the gate checks that the anchor exists in content we hold and cannot check that the link works**, so a reader following what reads as evidence would find nothing; and `CITATION_PATTERN` matched only `/validated-…`, so the authoring half was a **no-op** — a proposal citing an organisation's own standard would have carried "cites no pinned guidance that resolves" while citing exactly what it was told to follow. **Composing unconditionally broke ten answering rows** whose provider doubles distinguish guidance from estate by `isinstance(material, Corpus)`; they were right, and with nothing endorsed the pinned corpus is now passed **unchanged** — an estate that endorsed nothing runs the code it ran before. **Wired in both assemblies and proved on the served process**: the replaced API allocation created the endorsed tables under its own workload identity, and both new routes answer 401 unauthenticated. The allocation lane and the host lane each run half the enclave rows, because the store needs an attested identity and the transport needs `git`, which that image does not carry — naming the directory on one lane would have left the feature's single outbound operation exercised by nothing. **It also found that 040/041's `test_row_a22_the_postgres_leg` had never run** (a fixture that does not exist, erroring at setup one line above a passing summary); the fixture now exists. **Owed**: retention of superseded versions is deferred with its reasoning (a version is unreferencable only when no record cites it and no suspended run pins it — a query, not a TTL); the console half of EL1/EL2 needs an interactive sign-in and is the named runner's; MCP-server sources remain a separate feature |
| 048 | Ask and Build share one conversational shell | ADR-0034 (thin client — presentation; one additive `intake_message` field), ADR-0039 (never-acts remains visible as Ask status) | Designed-theme a11y lane (dark only). Identity, font, and composer-geometry rows. `intake_message` is null when `RunInput` is missing or unreadable. Human WCAG walk is the named runner's on the implementation PR. |
| 049 | Product-and-phase Build instructions | **ADR-0071 (new)** — GEPA/`dspy` are eval-lane only; served `src/core`, `src/adapters`, and `src/surfaces` never import them. ADR-0030 (executed `AGENTS.md` is pinned), ADR-0004 (provenance + lens + evals), ADR-0039 (Judge file on the judge cell), ADR-0047 (suites that can lose) | One pinned `packs/<pack>/agents/<phase>/AGENTS.md` per Build phase. Ask and the portal do not compose those files. Promotion is `promote_phase_agents` after `phase_agents` and `build_agents` (not `SUITES`). Named-runner SC-006 (25 Aug, n=5, Sonnet 5, Terraform 1.15.8): promoted Terraform Write **5/5 both gates** vs generic **5/5 tooling / 2/5 reference**. Joint DSPy unrun (needles would pull Write off authoring gates). Vault E3 is GEPA + 0.2.0 pins; the authoring corpus is Terraform-shaped. |
| 046 | An answer can be true, cited, on-subject and useless | ADR-0018 / ADR-0043 (consumed — usefulness is a separate question from grounding, and the relevance judge is left exactly as 043 measured it) | **The gap was never in the gate.** A customer's endorsed standard says logs are kept 400 days; the answer said the standard *defines* a retention period and did not say what it was. Every gate passed and each was right to: it resolves, it is on subject, and `anthropic_relevance.py`'s prompt says verbatim that a claim is relevant *"even if it is partial … or only says where the full answer is documented"* — a line 043 bought with live calls after Opus scored 7/10 refusing thin-but-on-subject claims. Tightening the judge re-breaks a measured result. The suites scored deny, decline, cited and estate references — existence, safety, scope, grounding — and none could express *"the reader now knows the retention period"* |
| 050 | Create home — one signed-in surface for Ask and Build | ADR-0034 (thin client), ADR-0028 (product identity — Nocturne, not a second copper) | Presentation only. Replaced 048's icon-rail split with one composer carrying an Ask/Build slider, combined searchable history, and the profile column. **Withdrew the Terraform plan gate** rather than carry a check the platform could not perform honestly — the same move 051 later generalised into declared unsatisfiable recommendations |
| 051 | Adopted skills reach the phase that needs them | **ADR-0004 (the consumption half it never had)**, ADR-0030 (executed content is pinned — and now actually executed), ADR-0003 (the binding is pack content, never source), ADR-0038 (what the platform cannot perform goes to the reviewer), ADR-0047 | **Two skills were pinned, digest-verified, named as practice by all five phase cards, and delivered to nobody.** `content_pins` recorded a digest for each, which reads as *this governed the run* and was true of none. Binding is now a `phases` array in the manifest; a bound phase's model receives the bytes, verified again at delivery, and nine distinct reason codes cover missing, empty, drifted and over-budget. **The record split in two**, because `content_pins` is written at `RUN_START` and can say what a pack binds but never what a run was steered by — and the per-phase map that answers that had been set since 049 and written nowhere. **The SC-006 row found the same defect in the Vault pack**, which research had recorded as clean; the earlier check grepped Terraform's wording. **SC-002 is NOT DEMONSTRATED and recorded as such**: three arms, forty model calls, validation blocks 5/5 both ways and shared tagging 0/5 both ways. Delivery is proven byte-for-byte; that receiving a skill changes the output on this corpus is not — the rules these skills teach are either already in the model or already hand-restated in the phase cards. The threshold was not moved |
| 052 | A finished authoring run leaves no proposal behind | **ADR-0024** (the durability seam is *used*, not widened — no provider method, because `save` already upserts by `blob_id`), ADR-0026 (the open/closed-bracket reasoning redone for a payload rather than inherited), ADR-0018 / ADR-0038 (the pull request is the durable artifact, which is what makes clearing the platform's copy defensible) | **041's FR-033 closed one of two copies.** The composed proposal — the same authored file bodies — also rested in `checkpoints.payload`, where six completed runs held ~81 KB of customer content. Found only because #220 corrected a sweep that had been matching the English word *"secret"* in a Judge model's prose for three weeks. **Retention and attestation did not trade**, because `provenance` already carried a path-and-digest line per file: a reviewer still hashes the merged pull request and matches it. Two analyze passes, and the second — which compared the artifacts to the **code** rather than to each other — found that the planned re-save would have blanked `correlation_id` on the terminal checkpoint, and that two contract rows contradicted each other. **Implementation found a third**: `usage` carried a shell transcript with a credential-shaped assignment, so the spec's "prose about the change, not an extract from it" line did not survive a real payload. **FR-011 ships open and recorded** — a run that never reaches terminal state is never scrubbed, which is the case holding content longest |
| 053 | A phase card delegates to the skill it is bound to | **ADR-0004 made load-bearing** (the pin governed nothing while the cards restated the guide by hand — Write 18 of its stated rules, Judge 13, Plan 8, so removing the binding would have left every one in force). ADR-0030 (one copy of executed content, not a pinned copy shadowed by a hand-maintained duplicate). ADR-0047 (a binding whose removal changes nothing is the passing stub in the content plane). Principle VII — one body of practice maintained in two places, one of them vendored and unmodifiable, already contradicting itself on `required_version`. **Zero core source changes.** Three declared overrides now say on the page what 051's precedence rule was resolving silently. **051's SC-002 answered rather than met**: its two measured rules were drawn from the guide's example code, so the guide never taught them and no delta was ever possible; re-measured on a prose-stated rule with genuinely different arms (17,740 bytes against 5,590) it came back 5/5 and 5/5 — the qualified model already organises files unaided. The minimality hypothesis is withdrawn as resting on a false premise. Left open, because it is a question about the criterion rather than the feature: a delegating card must name what it delegates, so a card-only arm always carries the rule's subject, and SC-002 may be unmeasurable by construction for any delegated rule |
| 054 | A run's write grant names only its own workspace | **ADR-0057's trigger 1 discharged** (it fired on 2026-08-07 with 042 and went unnoticed for three weeks; only the WRITE case re-opened, which that record's own Decision predicted). **ADR-0056 consumed and NOT needed** — 016's resource-server substrate stays parked, because Nomad's workload identity turned out to carry `nomad_allocation_id` and `user_claim` can name it. ADR-0058 is what closed the cheap fallback of handing a credential to the allocation. The defect was demonstrated (a run read, overwrote and deleted another run's measurement policy: 200/200/204) and the fix is demonstrated on the same terms (403/403/403 under real run authority, with a row that widens the estate to prove the safety case can lose). **AMENDED 2026-08-27 before merge: the first mechanism is withdrawn.** It cost one permanent Vault identity entity per Build against a documented hard ceiling of ~480,000 on integrated storage — reached in ~2.4 months at 10,000 users — and entity writes happen on every login, so the failure mode is that logins stop and every Build fails. The bound was correct; every gate asked whether it held and none asked what it cost to maintain. Replaced by moving the measurement to the long-lived surface, which keeps the isolation, meets FR-012 in full (a run holds NO policy-write authority), and adds nothing per run. Proposed **ADR-0072** — identity is per definition, never per invocation |
| 047 | Propose chat — repo URL to phased work to PR | ADR-0034, ADR-0033, ADR-0038/0064/0066 (consumed), **ADR-0047** (plan and judge are gates that can lose), ADR-0068 (plan evidence is the Terraform impact oracle, parallel to Vault’s measurement), ADR-0039 (Ask never acts) | Intake + live phases + fail-closed publish shipped in `#202` / follow-ups `#206`–`#208`. Remaining US4: the authoring allocation pins Terraform CLI 1.15.8; `terraform init` then `terraform plan` is the last analyzer gate; a red plan does not hand off a proposal; bounded plan evidence round-trips through the checkpoint onto the PR. Hermetic P3–P6 in `tests/conformance/propose/`. Enclave E1–E3 remain named-runner. |
| 044 | The admin console — governance configuration leaves Terraform | **ADR-0069 (new)** — governance configuration is requested at a console and decided by the trust fabric, moving [ADR-0026](docs/adr/0026-per-source-model-bindings.md)’s line at **origination only**; what decides a change is unchanged. **ADR-0016 consumed** (the quorum mechanism, reachable by a person for the first time), ADR-0025 (a dispatched run must not reach this write path), ADR-0039 (the closed role vocabulary — presented, not widened), ADR-0047, ADR-0067 (the judge toggle’s constraint) | **The mechanism had no principal.** `authority_submit.py` has shipped since 007 — surface requests, Control Groups decide, three outcomes mapped — and was **never exercisable in a deployment**: no policy granted write on any `harness-authority` path, `authority_change` was attached to no role, and the submitter takes no token. The conformance rows passed because they run with an operator token. 041’s shape one layer down, so the first task was infrastructure rather than a page. **`controlled_paths` never covered the path the gated writer writes to**, while the variable’s own description called it gated — fixed here, since FR-005 would otherwise be false wherever a quorum was configured. **Four defects the tooling and the live legs found, not review**: 042’s V6 scan caught this feature declaring two policies without protecting them — one of them the console’s own write grant, so a policy-authoring run could have widened what the console may change; `read_ask_binding` **unwraps** the KV record, so the CAS guard would have been silently `None` against the production fabric while every hermetic row passed; a fail-open `except` **hid a wiring bug** (a private attribute read as a method), so the toggle silently did nothing and five rows failed pointing elsewhere; and **Vault answers a failed check-and-set with 400, not 409** — the code checked 409 and the hermetic row scripted 409, a test agreeing with its author rather than with the product, so two administrators would have overwritten each other silently. **`relevance_note` never reached the caller** either: 043 put it on the answer and rendered it nowhere, which was survivable while it only said “a model judged this” and is not once it carries “this was not checked”. **Proven on a served surface**: the gate flipped in the fabric between three real asks with nothing restarted. **Owed**: ceilings, the matrix and the protected set stay estate governance; the `research`/`validate` role names were dropped rather than aliased, because a display alias invites belief in a capability that does not exist |
| 043 | Grounded means relevant, not merely resolvable | **ADR-0067 (new)** — a model does not judge its own output, binding in two places: qualification (a cell's `judge` may not name its own model) and **runtime** (a binding may not pair the answering model with the judge). The first pass implemented only the first, and the ADR's own text is what caught it. **ADR-0052 amended** (which model may judge, on top of how a judge earns its place), ADR-0022/0039 (the `judge` role carries its own qualification — an `ask` cell does not license it), ADR-0047, ADR-0033 (transport parity, which caught the gate reaching the API and not MCP) | **Citation resolution had been standing in for relevance since 024.** `Corpus.resolves` answers whether a document exists and carries the anchor; 035 widened the pin across six product families, and a question about subject X could then be answered from documents about subject Y with every citation resolving. A qualified judge now decides per claim, after resolution and before anything ships; nothing left means decline. `MODEL_GATE` got its **first production writer**. **The case was not edited** — `vault-must-decline-001` declines against a live model unchanged, and cross-product answers still answer, so the defect was not traded for its opposite. **Four findings the live legs produced and the hermetic rows could not**: a 256-token judge budget returned an EMPTY body on three consecutive samples, because the model spends budget reasoning before the verdict line — 031 recorded that exact failure and the adapter's own docstring cited it as a risk while the constant reproduced it; the judge had no way to resolve *"this platform"*, and the obvious fix (product statements do not answer such questions) corrected the case 3/3 and dropped the seed set from 10/10 to **6/10**, so what shipped changes deixis resolution alone; Opus scored **7/10** as a judge until the prompt distinguished SUBJECT from sufficiency — it was refusing on-subject claims for being thin, a stricter standard than this gate's — after which both candidates held 3/3 on the discriminating cases, which is the number that would have moved had the clarification loosened the gate; and smoke's answering legs **gated a different sample than they printed**, calling the provider twice. **The seed labels were not touched**, because revising a standard after watching a candidate fail it is the shape this estate refuses — and they turned out to be right. **R15's premise was falsified by its own measurement**: the rigged always-affirm candidate was predicted to clear the overall floor and fail only the second number, and it scores **40%**, failing both; the two-number design is justified instead by a candidate missing ONE supported-but-irrelevant case, which clears the floor and would be promoted by a single number. **Proven on the served process**, not only in rows: Opus judging Sonnet, recorded in the enclave's trail. **Owed**: `vault-must-decline-001` routes to *estate* on the deployed path (the router sees "audit log") so it declines there by visibility rather than by this gate — **demonstrated 2026-08-08**, when a documentation question phrased *"how long do we retain audit logs?"* was answered from run records, which is a confident well-formed answer to a question nobody asked; and the gate's own `subject, not sufficiency` line, which this feature measured and needs, is why an answer can now be true, cited, on-subject and useless (see [that entry](#an-answer-can-be-true-cited-on-subject-and-useless-unnumbered)), and the deixis handling is not uniform — *"the recommended upgrade path for this platform"* answers rather than declines |
...
**It happened a fourth time, corrected 2026-08-27.** 046, 050 and 051 had shipped with no
Shipped row, and 046 was still sitting under `Next` as *unnumbered* three weeks after it
merged. The file warns about exactly this defect three times in its own text, says it produced
two wrong recommendations in a single session, and was three features behind again. **A planner
reads this page first**; if a feature is not written down here on the day it merges, this
section is the thing that misleads them.
...
**047 (Propose chat) left this section on 2026-08-25.** Intake, phases, and fail-closed publish had already merged (`#202` and follow-ups). The remaining US4 work — Terraform CLI in the authoring allocation, plan-fail blocks the PR, bounded plan evidence on success — is in the Shipped table. Enclave E1–E3 stay named-runner.
...
Measured against merged main **as of 2026-08-06**, most of the machinery existed and none of
it composed yet. **047 closed the intake and the plan-as-gate path** (see Shipped). The
table below is the analysis as it stood then:
...
| read what exists in Terraform | `terraform_plan` in `PLATFORM_HANDLERS` | **closed for the authoring path by 047** — real `terraform init` + `terraform plan`; a missing binary or non-zero plan refuses rather than returning a fixture. Pack-tool Ask/run still uses `fixture_probe` because Terraform is not a federated estate product here |
| read what exists in context | pinned corpus; estate answering; **a customer's own endorsed documents (045)** | MCP-server sources remain open — the ROADMAP's own split |
...
**The same shape, second product — Vault policy authoring — SHIPPED by 042, 2026-08-07.**
The impact instrument turned out to be neither of the two options this table anticipated:
Vault has no plan-equivalent, and read-and-diff plus reasoning would have been a derived
answer. **042 asks the product**: the proposed policy is written under a throwaway name, a
token carrying only it is minted, `sys/capabilities` answers, and both are destroyed inside one
tool call (ADR-0068). The paragraph below is the analysis as it stood before that was built.

**(Original entry, added 2026-08-06.)** *What exists?
...
### The admin interface — governance leaves Terraform — SHIPPED by 044, 2026-08-07

**Closed by [`specs/044-admin-governance-console`](specs/044-admin-governance-console/).** An
administrator reads the platform's posture and requests changes to it from `/settings`; the
trust fabric decides. Three records are writable — the ask bindings (including the
LLM-as-a-judge toggle), claim mappings, and a new product-connections record — through 007's
existing three-outcome submitter. **The sharpest question below is answered**: a disabled judge
*discloses*, on every answer it would have checked ([ADR-0069](docs/adr/0069-governance-configuration-is-requested-at-a-console.md)),
and that answer is the template for every toggle after it. The analysis as it stood before the
feature follows.

**(Original entry, raised 2026-08-07.)**
...
context](#customer-supplied-context--git-repo-half-closed-by-045)'s *"endorsement by an admin, configured in the
...
### An answer can be true, cited, on-subject and useless — SHIPPED as 046

**Left this section on 2026-08-27**, three weeks after it shipped. Kept below because the reasoning is still the reasoning and the measurements it cites are still the ones anybody proposing a stricter judge has to read first.

Raised 2026-08-08, from a walkthrough rather than from review. Recorded with its measurements
because the tempting fix is the one 043 already tried and measured against.

**What happened.** A customer's own standard was endorsed, synced and adopted, and the question
*"how long are logs kept?"* was answered:

> Acme maintains its own internal standard that defines how long operational logs are kept.

The document says **400 days**. The answer does not. It is true, it resolves, it is on subject,
and it tells the reader nothing they did not already know — they asked *because* they assumed a
standard existed.

**Every gate passed, and each was right to.** The ask record shows `guidance` /
`answered` / `relevance_gate: checked`, resting on exactly one endorsed version. Citation
resolution proved the section exists. The relevance judge was asked whether the claim is about
what was asked, and it is.

**The gate is doing what it was instructed to do, and the instruction was bought with live
calls.** `anthropic_relevance.py`'s prompt says, verbatim:

> Judge SUBJECT, not sufficiency. A statement that is about what was asked is relevant even if
> it is partial, or general, **or only says where the full answer is documented**.

That last clause describes this answer exactly. And the line is not incidental: 043 added it
after Opus scored **7/10** as a judge by refusing on-subject claims for being thin, and
`test_live_relevance_adapter.py` pins the numbers. **Tightening the gate re-breaks 043 in a way
that was already measured.** Anyone proposing "make the judge stricter" should read that
feature's record first.

**So the gap is not in the gate. It is that nothing anywhere asks whether the answer is
USEFUL.** The four eval suites score `deny`, `decline`, `cited`, and estate references —
existence, safety, scope and grounding. None of them can express *"the reader now knows the
retention period."* A suite of true-but-useless answers would score full marks today.

**Why this is worse than the reachability defects found the same day.** An unlinked page and an
unauthenticated writer are loud: nothing happens, and you go looking. This is quiet. It returns
something answer-shaped, in the platform's own confident register, with a citation attached —
and the reader's only defence is following the link and reading the document themselves, which
is the work the platform exists to save.

**What a specify must settle, and the candidates are genuinely different features:**

- **A second question at the gate.** Ask *does this answer the question* separately from *is
  this about the question*, keeping the subject check exactly as it is. Cheap and additive —
  and it is a **model judging sufficiency**, which is the judgement 043 measured as unreliable
  in the direction of over-refusal. It also doubles the judge calls per ask.
- **A retrieval problem, not a gate problem.** The model cited a preamble when a sibling section
  held the figure. Nothing scores *which* section was chosen when several resolve, and section
  granularity is decided by the sync's heading split — so the fix might be upstream of answering
  entirely. **Measure this first**: if the answering model reliably picks the summary over the
  substance, no gate downstream will rescue it.
- **A fifth eval suite.** `answer_sufficiency`, scored against cases carrying the fact the
  answer must contain. It measures rather than enforces, which fits the estate's posture — but
  it needs a case shape the current four cannot express, and `report_fidelity` already
  established that forcing a new shape into `expected: str` loses the signal.
- **Say less, more honestly.** An answer that only locates the information could disclose that
  it does — *"your standard covers this; see logging.md"* — rather than reading as the answer.
  Smallest change, no new model call, and it does not make the answer better.

**Do not start by tightening the judge.** That is the one option whose cost is already on
record, and the seed-set number that would move is written down.

**The document is not an unfair case.** A heading, a summary sentence, then the figure further
down is how standards are actually written — and the summary sentence is the one that most
resembles an answer.

### Customer-supplied context — Git-repo half CLOSED by 045

**A customer's own documents are endorsed, synced, pinned and citable, and every answer says
whose material it rests on.** An administrator endorses a source through 044's request-and-decide
path; the platform clones it, addresses it by heading, and writes an immutable content-addressed
version into the harness Postgres; citations into `/endorsed/<source>/…` resolve against that
version by the same check the pinned corpus uses; and each citation carries
`provenance: validated-design | customer-endorsed` as data. Drift is detected by a refs listing
in the existing health checker — **noticing changes nothing** — and adoption is a person's act
with a name and a time against it. A run in flight keeps the version it started on, across
resume. **ADR-0070** adds endorsed-content sync as an enumerated egress class with its bounds,
and settles ADR-0030's tension: customer content is *consulted* material handled by the *pinned*
mechanism.

**Research R1 answered the design question this entry raised.** Not a tenant dimension on
`corpus.py` — a **second, parallel corpus** behind the same resolution contract, so the check
this platform calls *"the single most important"* is not extended to content the platform does
not control. `corpus.py` is unedited, asserted as a diff from the feature's first commit.

**The MCP-server half is still open**, exactly as this entry predicted, and everything below it
remains the analysis to build on.
...
**The Git-repo half shipped as 045** — see the note at the top of this entry. What remains is the
MCP-server half, **not scheduled**, and the analysis above is what it should be built on rather
than re-derived. **The first question its specify must answer**: do customer MCP servers supply
*context* or *capability*? Everything above sizes differently depending on the answer, and the
phrase *"additional context or data"* suggests the first while *"MCP server"* usually delivers
both.

045 also narrowed two of the three things this entry said needed deciding. **Credentials** is
settled for sources: a private source's material is trust-store material referenced per sync and
never entered through the console (044's FR-018b posture, unchanged). **Freshness** is settled for
repositories and remains open for live servers, which is the distinction this entry drew: you can
digest-pin a clone and you cannot pin a query interface. **Eval gating** is untouched and is
still the ADR that half needs.
...
**0g was different from the six above it**, and the difference is worth stating: those were gaps
between what the record claimed and what the code did. 0g was a gap in what the platform will
**assert to a person**, and it held the live lane red until 043 closed it.

### 0g. A resolving citation proves a document exists, not that it answers the question — **CLOSED by 043, 2026-08-07**

**Closed by [`specs/043-grounded-means-relevant`](specs/043-grounded-means-relevant/): a
qualified judge model decides, per claim, whether it answers the question that was asked, after
citation resolution and before anything ships.** Resolution still gates existence; relevance is
a second, separate check, and an answer with nothing left declines *"the corpus does not cover
what was asked"* rather than answering from the neighbours. `MODEL_GATE` records the verdict —
which model judged, how many claims were kept and dropped — and got its first production writer
in the process.

**The case was not edited.** `vault-must-decline-001` declines against a live model with its
text unchanged, and `make evals-smoke` exits 0. The cross-product answer 035 widened the corpus
for still answers: nothing here scopes a pack to its own product's documents, which is the fix
that would have traded this defect for its opposite.

**The judge is not the model being judged** (ADR-0067, written in the same feature): the dev
estate answers with Sonnet 5 and judges with Opus 5, and a binding pairing them is refused
`self_judged_relevance` at resolution. Opus qualified against the human-labelled relevance seed
set at majority-of-three on two numbers — overall agreement against a ≥90% floor, and the
supported-but-irrelevant cases which must be *all* correct. The same lane refuses an
always-affirming stub at 40%, so the qualification can lose.

**Two limits, measured on the deployed surface and left open rather than claimed.**
`vault-must-decline-001` routes to *estate* on the served path — the router sees "audit log" —
so it declines there through visibility rather than through this gate; the gate's decline on
that case is proven on the answering path and in the hermetic rows. And the "this platform"
deixis handling is not uniform: *"what is the recommended upgrade path for this platform?"*
answers rather than declines. Both are narrower than 0g and neither reopens it.

The account of the defect, as found, follows.
