...
| 049 | Product-and-phase Build instructions | **ADR-0071 (new)** — GEPA/`dspy` are eval-lane only; served `src/core`, `src/adapters`, and `src/surfaces` never import them. ADR-0030 (executed `AGENTS.md` is pinned), ADR-0004 (provenance + lens + evals), ADR-0039 (Judge file on the judge cell), ADR-0047 (suites that can lose) | One pinned `packs/<pack>/agents/<phase>/AGENTS.md` per Build phase. Ask and the portal do not compose those files. Promotion is `promote_phase_agents` after `phase_agents` and `build_agents` (not `SUITES`). Live GEPA/DSPy and SC-006 are named-runner (Dan McTeer). |
