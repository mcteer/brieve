...
| Production-shaped card (`AGENTS.production.md`, no FILE protocol) | tooling **4/5**, reference **5/5**. Least-privilege wildcard miss is gone. One authored tree failed `terraform validate`. Both-gates-complete still false. |
| Delta vs generic | GEPA FILE card: reference **+3/5**. Production-shaped: reference **+4/5** (20% → 100%) but tooling **−0** vs a 4/5 generic (generic was 4/5 tooling this shot). Strictly higher reference pass rate on both overlays. |
| Promoted? | **No.** Whole-set rule; production-shaped card is a hand-stripped GEPA descendant, not a five-file compile; both-gates-complete still false. |

A non-positive delta is a failed eval. These named-runner shots are a positive reference-gate delta and an incomplete corpus (one `terraform validate` miss on the production-shaped card). E3 stays open until a promoted, production-shaped **five-file** set beats generic at the same n and both gates complete. E1: connected, restricted, and air-gapped profiles execute the same pinned files (no public-web fetch at phase start).
